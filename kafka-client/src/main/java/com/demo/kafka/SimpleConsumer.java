...
class SimpleConsumer extends AbstractSimpleKafka{
    ...
    void run(String topicName, KafkaMessageHandler callback, Integer numberOfRecords) throws Exception {
        Properties props = PropertiesHelper.getConsumerProperties();
        ...
        while (recNum > 0) {
            ...
            if (recNum == 0) {
                log.info(MessageHelper.getSimpleJSONObject("consumer", "No records retrieved"));
                break;
            }
    ...
    private void close() throws Exception {
        if (this.getKafkaConsumer() == null){
            log.info(MessageHelper.getSimpleJSONObject("consumer","The internal consumer is NULL"));
            return;
        }
        log.info(MessageHelper.getSimpleJSONObject("consumer","Closing consumer"));
    ...
    public void shutdown() throws Exception {
        closed.set(true);
        log.info(MessageHelper.getSimpleJSONObject("consumer","Shutting down consumer"));
